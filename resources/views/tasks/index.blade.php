          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr role="row">
                ...
                <th width="12%" class="sorting hidden-xs" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="HiredDate: activate to sort column ascending">Attachment</th>
                <th width="8%" class="sorting hidden-xs" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Department: activate to sort column ascending">Deadline</th>
            ...
            @foreach ($tasks as $task)
                <tr role="row" class="odd">
                  ...
                  <td class="hidden-xs">{{ $task->attachment }}</td>
                  <td class="hidden-xs">{{ $task->deadline }}</td>
                 
                  <td>
                    <form class="row" method="POST" action="{{ route('employees.destroy', ['id' => $task->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('employees.edit', ['id' => $task->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
